...
/**
 * spec.md 第4節の保護対象ルート一覧が、明示的に `- NONE` と宣言されているか。
 *
 * 「読めない・空」と「無いと宣言してある」は別物である。前者は検査できて
 * いないので落とす。後者は契約として宣言されており、scripts/check-catastrophic.sh
 * も同じ扱い（SKIP を表示して継続）をしている。両者を混ぜない。
 */
export function protectedRoutesDeclaredNone(specPath: string): boolean {
  if (!existsSync(specPath)) return false;
  const lines = readFileSync(specPath, 'utf8').split(/\r?\n/);
  let inSection4 = false;
  let afterMarker = false;
  for (const line of lines) {
    if (/^##\s+4[.．]/.test(line)) { inSection4 = true; continue; }
    if (!inSection4) continue;
    if (/^#{1,2}\s/.test(line)) break;
    if (line.includes('保護対象ルート一覧')) { afterMarker = true; continue; }
    if (afterMarker && /^\s*-\s*NONE\s*$/.test(line)) return true;
  }
  return false;
}
...
const WISH_1 = 'うちの美容室の予約を、電話じゃなくてネットで受けたい。';
const WISH_2 = '店長がやっている月末の売上集計を、画面で終わらせたい。';

/** ダウンロードされたファイルの中身を文字列で読む。 */
async function readDownload(d: Download): Promise<string> {
  const path = await d.path();
  if (path === null) throw new Error('ダウンロードの保存先が取れなかった');
...
test.describe.serial('受入検査（1回のブラウザ起動に束ねる）', () => {
  ...
  // -------------------------------------------------------------------------
  test('① ハッピーパス1周', async () => {
    // [転写1] spec.md 第2節 B-1 Given — 何も準備していない人が、画面を開いている
    // この案件はログインを持たない（spec.md 第4節「権限: なし」）。
    await page.goto('/');
    // [転写1 ここまで]
    ...
    // [転写3] spec.md 第2節 B-1 Then — Markdown が1個だけ落ち、中に4つが入り、次の1文が出る
    expect(download1.suggestedFilename()).toMatch(/\.md$/);
    const body1 = await readDownload(download1);
    expect(body1).toContain(WISH_1);                    // 書いた文がそのまま入っている
  ...
  test('② 画面への引っ掛け', async () => {
    for (let i = 0; i < UI_HOOK_COUNT; i++) {
      ...
      try {
        ...
        // ダウンロードの完了は待たない。空文字など、仕様上ダウンロードが
        // 起きない入力が混じるため（spec.md 第2節 B-4）。ここで見るのは
        // 内部エラーが画面に出ないことだけである。
        await page.goto('/');
  ...
  test('③ API層への引っ掛け', async () => {
    // この案件はサーバへ何も送らない（spec.md 第3節「書いた内容がサーバに届かない」）。
    // API の入口が存在しないため、引っ掛ける対象が無い。件数を減らして緑にしたのではなく、
    // 検査対象そのものが無い。存在しない対象を検査したことにしない。
    console.log('SKIP ③ API 層が存在しないため検査対象が無い（spec.md 第3節「書いた内容がサーバに届かない」）');
    if (API_HOOK_COUNT === 0 || true) return;
  ...
  test('④ 2周目', async () => {
    try {
      // [転写2 の再掲] spec.md 第2節 B-2 — 別の内容で押すと、もう1個落ち、前のは変わらない
      const before = await readDownload(download1);
      await page.goto('/');
      ...
      expect(body2).not.toContain(WISH_1);
      expect(await readDownload(download1)).toBe(before);  // 先のファイルは変わっていない
      // [転写2 の再掲 ここまで]
    } catch (e) {
  ...
  test('⑤ 権限', async () => {
    ...
    if (routes.length === 0) {
      if (protectedRoutesDeclaredNone(SPEC_PATH)) {
        // 契約として「保護対象なし」が宣言されている（spec.md 第4節「権限: なし」）。
        // 黙って緑にしているのではない。宣言を変えれば、ここは即座に検査対象になる。
        console.log('SKIP ⑤ 保護対象ルートが - NONE と宣言されているため検査対象が無い');
        return;
      }
      record(
        '⑤',
    ...
    try {
      // [転写6] この案件はログインを持たない（spec.md 第4節「権限: なし」）。
      // 別人としてログインする経路が無いため、ここには手順を書けない。
      // [転写6 ここまで]
